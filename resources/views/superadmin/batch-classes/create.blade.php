        ...
        <div class="row align-items-center">
            ...
            <div class="col-md-12">
                <div class="page-header-title">
                    <h2 class="mb-0">Schedule New Class</h2>
                </div>
            </div>
        </div>
    </div>
...
<div class="row">
    <div class="col-lg-10 mx-auto">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('superadmin.batch-classes.store') }}" method="POST">
                    @csrf
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('master_class_id') is-invalid @enderror" id="master_class_id" name="master_class_id" required>
                                ...
                                @foreach($masterClasses as $masterClass)
                                    <option value="{{ $masterClass->id }}" {{ old('master_class_id') == $masterClass->id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" class="form-control @error('date') is-invalid @enderror" id="date" name="date" value="{{ old('date', $selectedDate ?? '') }}" required>
                            @error('date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" id="is_have_gender_restriction" name="is_have_gender_restriction" value="1" {{ old('is_have_gender_restriction') ? 'checked' : '' }}>
                            <label class="form-check-label" for="is_have_gender_restriction">
                                Enable Gender Restriction
                            </label>
                        </div>
                        <div id="gender_restriction_field" style="display: {{ old('is_have_gender_restriction') ? 'block' : 'none' }};">
                            <select class="form-select @error('gender_restriction') is-invalid @enderror" id="gender_restriction" name="gender_restriction">
                                ...
                                <option value="Women" {{ old('gender_restriction') == 'Women' ? 'selected' : '' }}>Women Only</option>
                                <option value="All Gender" {{ old('gender_restriction') == 'All Gender' ? 'selected' : '' }}>All Gender</option>
                                <option value="Men" {{ old('gender_restriction') == 'Men' ? 'selected' : '' }}>Men Only</option>
                            </select>
                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" id="is_have_age_restriction" name="is_have_age_restriction" value="1" {{ old('is_have_age_restriction') ? 'checked' : '' }}>
                            <label class="form-check-label" for="is_have_age_restriction">
                                Enable Age Restriction
                            </label>
                        </div>
                        <div id="age_restriction_field" style="display: {{ old('is_have_age_restriction') ? 'block' : 'none' }};">
                            <input type="text" class="form-control @error('age_restriction') is-invalid @enderror" id="age_restriction" name="age_restriction" value="{{ old('age_restriction') }}" placeholder="e.g., 18+, 13-17, All Ages">
